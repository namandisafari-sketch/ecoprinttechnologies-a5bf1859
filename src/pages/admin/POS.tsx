...
const AdminPOS = () => {
  ...
  const processSale = async () => {
    ...
    try {
      // sale_number is auto-generated by the database default
      ...
      const saleData: any = {
        ...
        total,
        payment_method: paymentMethod === 'wallet' ? 'cash' : paymentMethod,
        ...
        notes:
          paymentMethod === 'wallet'
            ...
            : saleDate
            ? `Backdated sale from ${format(saleDate, 'PPP')}`
            : null,
      };
      ...
      if (paymentMethod === 'credit' && selectedCustomerId) {
        ...
        const { error: creditError } = await supabase.from('credit_sales').insert({
          sale_id: sale.id,
      ...
      await logAudit({ action: 'create', entityType: 'sale', entityId: sale.id, description: `POS sale ${sale.sale_number} — ${cart.length} item(s), total ${total}`, metadata: { payment_method: paymentMethod, total } });
      setShowReceipt(true);
      setCart([]);
      setAmountPaid('');
      setCustomerName('');
  ...
  const endDay = async () => {
    ...
    try {
      ...
      const { error: depositError } = await supabase.from('bank_deposits').insert({
      ...
      const { data: existingRegister } = await supabase.from('cash_register').select('*').eq('date', dateStr).maybeSingle();
                    ...
                    <PopoverContent className="w-auto p-0" align="start">
                      ...
                      {saleDate && (
                        <div className="p-2 border-t">
                          <Button variant="ghost" size="sm" className="w-full" onClick={() => setSaleDate(undefined)}>
                            Clear (use today)
                          </Button>
                        </div>
                      )}
            ...
            <div className="space-y-4 text-sm" id="receipt">
              <div className="flex items-start gap-3 border-b pb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Receipt #{lastSale.sale_number}</p>
              ...
              <div className="flex flex-col items-center pt-4 border-t space-y-3">
                <QRCodeSVG
                  value={`${window.location.origin}/admin/sales?receipt=${lastSale.sale_number}`}
                  size={80}
                  level="M"
                />
          ...
          <div className="space-y-2">
            ...
            <Popover>
              ...
              <PopoverContent className="w-auto p-0" align="start">
                ...
                {endDayDate && (
                  <div className="p-2 border-t">
                    <Button variant="ghost" size="sm" className="w-full" onClick={() => handleEndDayDateChange(undefined)}>
                      Clear (use today)
                    </Button>
                  </div>
                )}
