...
const renderQR = (url: string, size: number) =>
  `<img src="https://api.qrserver.com/v1/create-qr-code/?size=${size}x${size}&data=${encodeURIComponent(url)}" style="width:${size}px;height:${size}px;" />`;

const renderSingleSticker = (sticker: StickerData, heightMm: number): string => {
  ...
  const footerMarginTop = L.bodyFooterGapMm > 0 ? "0" : "auto";
  const gapSpacer = L.bodyFooterGapMm > 0 ? `<div style="height:${L.bodyFooterGapMm}mm;flex-shrink:0;"></div>` : "";

  return `
    <div style="width:100%;height:${heightMm}mm;padding:${L.paddingTopMm}mm ${L.paddingHorizontalMm}mm;display:flex;flex-direction:column;align-items:${titleAlignFlex};overflow:hidden;box-sizing:border-box;">
      ...
      ${gapSpacer}
      ${L.footerLayout === "grid" ? `
      <div style="margin-top:${footerMarginTop};padding-bottom:3mm;width:92%;">
      ...
      ` : `
      <div style="margin-top:${footerMarginTop};padding-bottom:3mm;width:92%;display:flex;align-items:center;justify-content:${footerAlignFlex};gap:${L.footerGapMm}mm;flex-wrap:wrap;">
      ...
      <head>
        ...
        <style>
          ...
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          .col { display: flex; flex-direction: column; height: 297mm; }
          .col + .col { border-left: 1px dashed #999; }
          .col { border-left: 1px dashed #999; }
          .col:last-child { border-right: 1px dashed #999; }
          .split-divider { border-top: 1px dashed #999; }
        </style>
      </head>
      <body>
        ${stickers.map((sticker) => {
          ...
          const halfHeight = (297 - 2) / 2;
          return `
          <div class="col" style="width:${L.stickerWidthMm}mm;">
            ${splitTwoUp
              ? `${renderSingleSticker(sticker, halfHeight)}<div class="split-divider"></div>${renderSingleSticker(sticker, halfHeight)}`
              : renderSingleSticker(sticker, 297)}
          </div>
          `;
        }).join("")}
      </body>
